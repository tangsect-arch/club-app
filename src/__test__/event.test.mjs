...
describe("Club API Tests", () => {
  ...
  it("should not register duplicate entry", async () => {
    ...
    expect(response.body.message).toBe(
      "Club with the same location and date already exists."
    );
  });
});
...
describe("Club API Tests", () => {
  it("should create a new event", async () => {
    ...
    expect(response.body.message).toBe("Club updated successfully");
  });
});

describe("ClubEvent API Tests", () => {
  it("should create new event seating", async () => {
    const response = await request(app)
      ...
      .expect(201);
    expect(response.body.message).toBe("Club seating created successfully");
  });
  ...
  it("should not register duplicate seating entry", async () => {
    ...
    expect(response.body.message).toBe("Club seating already exist");
  });
